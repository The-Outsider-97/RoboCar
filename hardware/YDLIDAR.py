...
class YdLidarX4:
    ...
    def StartScanning(self):
        ...
        if(self._is_connected):
            if(not self._is_scanning):
                ...
                if("YdLidarX4" in str(type(self))):
                    self._Start_motor()
                self._s.write((chr(0xA5)+chr(0x60)).encode())
                sleep(0.5)
                self._s.read(7)
                distdict = {}
                countdict = {}
    ...
    def StopScanning(self):
        ...
        if(self._is_connected):
            if(self._is_scanning):
                self._is_scanning = False
                self._s.write((chr(0xA5)+chr(0x65)).encode())
                sleep(1)
                self._s.reset_input_buffer()
                if("YdLidarX4" in str(type(self))):
                    self._Stop_motor()
    ...
    def GetHealthStatus(self):
        ...
        if(self._is_connected):
            ...
            self._s.reset_input_buffer()
            sleep(0.5)
            self._s.write((chr(0xA5)+chr(0x91)).encode())
            sleep(0.5)
    ...
    def GetDeviceInfo(self):
        ...
        if(self._is_connected):
            ...
            self._s.reset_input_buffer()
            sleep(0.5)
            self._s.write((chr(0xA5)+chr(0x90)).encode())
            sleep(0.5)
    ...
    def Reset(self):
        """Reboots the Lidar."""
        if(self._is_connected):
            self._s.write((chr(0xA5)+chr(0x40)).encode())
            sleep(0.5)
            self.Disconnect()
            self.Connect()
        else:
...
class YdLidarG4(YdLidarX4):
    ...
    def EnableLowPowerMode(self):
        ...
        if(self._is_connected):
            if self._is_scanning == True:
                self.StopScanning()
            self._s.write((chr(0xA5)+chr(0x01)).encode())
    ...
    def DisableLowPowerMode(self):
        ...
        if(self._is_connected):
            if self._is_scanning == True:
                self.StopScanning()
            self._s.write((chr(0xA5)+chr(0x02)).encode())
            while(self._s.inWaiting()==0):
                sleep(0.5)
            self._s.reset_input_buffer()
        else:
    ...
    def GetLowPowerModeStatus(self):
        ...
        if(self._is_connected):
            ...
            self._s.reset_input_buffer()
            sleep(0.5)
            self._s.write((chr(0xA5)+chr(0x05)).encode())
            sleep(1)
            data = self._s.read(8)
            if(data[7] != 1):
                return True
    ...
    def IncreaseCurrentFrequency(self,frequencyStep):
        ...
        if(self._is_connected):
            if self._is_scanning == True:
                self.StopScanning()
            if(frequencyStep==1):
                self._s.write((chr(0xA5)+chr(0x09)).encode())
            elif(frequencyStep==2):
                self._s.write((chr(0xA5)+chr(0x0B)).encode())
            while(self._s.inWaiting()==0):
                sleep(0.5)
            self._s.reset_input_buffer()
        else:
    ...
    def DecreaseCurrentFrequency(self,frequencyStep):
        ...
        if(self._is_connected):
            if self._is_scanning == True:
                self.StopScanning()
            if(frequencyStep==1):
                self._s.write((chr(0xA5)+chr(0x0A)).encode())
            elif(frequencyStep==2):
                self._s.write((chr(0xA5)+chr(0x0C)).encode())
            while(self._s.inWaiting()==0):
                sleep(0.5)
            self._s.reset_input_buffer()
        else:
    ...
    def GetCurrentFrequency(self):
        ...
        if(self._is_connected):
            ...
            self._s.reset_input_buffer()
            sleep(0.5)
            self._s.write((chr(0xA5)+chr(0x0D)).encode())
            sleep(0.5)
    ...
    def EnableConstantFrequency(self):
        ...
        if(self._is_connected):
            if self._is_scanning == True:
                self.StopScanning()
            self._s.write((chr(0xA5)+chr(0x0E)).encode())
            while(self._s.inWaiting()==0):
                sleep(0.5)
            self._s.reset_input_buffer()
        else:
    ...
    def DisableConstantFrequency(self):
        ...
        if(self._is_connected):
            if self._is_scanning == True:
                self.StopScanning()
            self._s.write((chr(0xA5)+chr(0x0F)).encode())
            while(self._s.inWaiting()==0):
                sleep(0.5)
            self._s.reset_input_buffer()
        else:
    ...
    def SwitchRangingFrequency(self):
        ...
        if(self._is_connected):
            if self._is_scanning == True:
                self.StopScanning()
            self._s.write((chr(0xA5)+chr(0xD0)).encode())
            while(self._s.inWaiting()==0):
                sleep(0.5)
            self._s.reset_input_buffer()
        else:
    ...
    def GetCurrentRangingFrequency(self):
        ...
        if(self._is_connected):
            ...
            self._s.reset_input_buffer()
            sleep(0.5)
            self._s.write((chr(0xA5)+chr(0xD1)).encode())
            sleep(1)
            data = self._s.read(8)
            if(data[-1]==0):
                return 4
...
if __name__ == "__main__":
    ...
    print(f"YDLIDAR test on port {PORT}")
    # Try X4
    lidar = YdLidarX4(PORT)
    try:
        if lidar.Connect():
            ...
            print("Starting scanning for 2 seconds...")
            gen = lidar.StartScanning()
            start_time = time.time()
            for scan in gen:
                # print first few angles
                print(f"Scan received, {len(scan)} points, first 5: {list(scan.items())[:5]}")
